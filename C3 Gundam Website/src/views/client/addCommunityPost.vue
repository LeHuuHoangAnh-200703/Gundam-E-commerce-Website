...
const handleFileUpload = (event) => {
    ...
    formData.value.image = [...formData.value.image, ...files];
    event.target.value = '';
};
                            ...
                            <form @submit.prevent="addCommunityPost" method="POST"
                                ...
                                <div class="md:col-span-5 mb-2 flex flex-col gap-2">
                                    ...
                                    <select v-model="formData.category"
                                        class="p-2 border font-medium mt-1 rounded px-4 w-full bg-transparent"
                                        name="" id="category">
                                        <option value="" class="text-[#003171] font-semibold">Chọn loại bài đăng
                                            phù hợp</option>
                                        <option value="Assemble" class="text-[#003171] font-semibold">Hướng dẫn lắp ráp</option>
                                        <option value="Collect" class="text-[#003171] font-semibold">Gundam sưu tầm</option>
                                        <option value="GundamNew" class="text-[#003171] font-semibold">Gundam mới ra mắt</option>
                                        <option value="Custom" class="text-[#003171] font-semibold">Custom Gundam</option>
                                    </select>
                                    <p v-if="errors.category" class="text-red-500 text-sm my-2">{{ errors.category }}</p>
                                </div>
                                <div class="md:col-span-5 mb-2 flex flex-col gap-2">
                                    <label for="image_upload" class="text-[15px] font-semibold text-white">Hình ảnh <i class="fa-solid fa-circle-info text-gray-300"></i></label>
                                    <label for="image_upload"
                                        ...
                                        <input type="file" class="hidden" name="image" multiple id="image_upload" @change="handleFileUpload"/>
                                ...
                                <div class="md:col-span-5 text-right mt-2">
                                    <div class="inline-flex items-end">
                                        <button type="submit"
                                            class="bg-[#333f48] hover:bg-[#DC143C] text-white font-bold py-2 px-4 rounded">Đăng bài</button>
                                    </div>
                                </div>
                            </form>
                        </div>
