...
const editProductType = async () => {
    ...
    if (!formData.value.nameProductType) {
        errors.value.nameProductType = "Tên loại sản phẩm không để trống!";
    } else {
        formData.value.nameProductType = escapeHtml(formData.value.nameProductType);
    }

    if (!formData.value.productType) {
        errors.value.productType = "Loại sản phẩm không để trống!";
    } else {
    ...
    try {
        ...
        showNotification(error.response?.data?.message || "Chỉnh sửa nhà cung cấp thất bại! Vui lòng kiểm tra lại thông tin.", "error");
    }
    setTimeout(() => {
        notification.value.message = '';
    }, 2000);
...
onMounted(() => {
    const maLoaiSanPham = router.currentRoute.value.params.maLoaiSanPham;
    fetchProductType(maLoaiSanPham);
})
</script>
            ...
            <div class="flex-1 px-4 py-4 overflow-y-auto">
                <div class="flex flex-col gap-4">
                    ...
                    <div class="bg-white rounded-lg shadow-lg w-full lg:w-[70%] mx-auto p-4">
                        <form @submit.prevent="editProductType" method="POST">
